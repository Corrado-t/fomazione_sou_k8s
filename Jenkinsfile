        ...
        stage('Using helm to install on kubernates cluster') {
            steps {
                script {
                    ...
                    sh "helm upgrade --install --kubeconfig=/var/jenkins_home/.kube/kubeconfig-minikube \
                     flask-app ./flask-app-chart --kube-insecure-skip-tls-verify --set tag=${env.DOCKER_TAG} --debug"  
                }
            }
        }        
